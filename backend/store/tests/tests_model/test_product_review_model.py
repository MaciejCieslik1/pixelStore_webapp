...
class TestProductReviewModel(unittest.TestCase):
    ...
    def test_eq_same_data(self):
        now=datetime.now()
        transaction1 = Transaction(buyer=self.buyer, total_price=1000, date_time=now, is_finished=False)
        transaction2 = Transaction(buyer=self.buyer, total_price=1000, date_time=now, is_finished=False)

        self.assertEqual(transaction1, transaction2)

    def test_eq_different_data(self):
        transaction1 = Transaction(buyer=self.buyer, total_price=1000, date_time=datetime.now(), is_finished=False)
        transaction2 = Transaction(buyer=self.buyer, total_price=1000, date_time=datetime.now(), is_finished=True)
