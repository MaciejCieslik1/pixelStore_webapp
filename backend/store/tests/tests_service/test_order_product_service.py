...
class TestFindByIdOrderProductService:
    def setup_method(self):
        ...
        product = Product(owner=self.seller, name="fefeffe", description="fefeffe", amount=1, price=item_price, color="red",
            weight=2, length=3, width=3, height=3, guarantee_period=1, status="available")
        transaction.save()
        product.save()
        ...
        self.order_product_data = {"order_product_id": 1, "transaction_id": 1, "product_id": 1, "seller_username": "tester2",
            "shopping_price": "1000.00"}

    def test_find_by_id(self):
        order_product_id = self.order_product.order_product_id
    ...
    def test_find_by_id_invalid_id(self):
        order_product_id = self.order_product.order_product_id + 1
        order_products_before = OrderProduct.objects.count()
...
class TestCreateOrderProductService:
    def setup_method(self):
        ...
        item_price = 1000.00
        self.transaction = Transaction(buyer=self.user, total_price=item_price, date_time=timezone.now(), is_finished=False)
        self.product = Product(owner=self.seller, name="fefeffe", description="fefeffe", amount=1, price=item_price,
            color="red", weight=2, length=3, width=3, height=3, guarantee_period=1, status="available")
        self.transaction.save()
        self.product.save()
        self.order_product_creation_data = {"product_id": self.product.product_id, "transaction_id": self.transaction.transaction_id,
            "seller_username": self.seller.username, "shopping_price": "1000.00"}
    ...
    def test_create(self):
        ...
        product = Product.objects.get(product_id=self.product.product_id)
        order_products_after = OrderProduct.objects.count()
    ...
    def test_create_user_is_not_owner_of_transaction(self):
        user3_data = {"email": "test3@example.com", "username": "tester3", "password": "Abc123#ab",
                            ...
                            "last_login": None, "address": "fweffwe", "postal_code": "00001", "city": "Warsaw",
                            "country": "Poland"}
        token3 = AuthenticationHelper.register_and_login_user(user3_data)
        user3 = User.objects.get(username=user3_data["username"])
        order_products_before = OrderProduct.objects.count()
        transactions_before = OrderProduct.objects.count()

        with pytest.raises(InvalidInputData) as e:
            self.service.create(token3, user3, self.order_product_creation_data)
        product = Product.objects.get(product_id=self.product.product_id)
        order_products_after = OrderProduct.objects.count()
        transactions_after = OrderProduct.objects.count()
    ...
    def test_create_finished_transaction(self):
        self.transaction.is_finished = True
        self.transaction.save()
        order_products_before = OrderProduct.objects.count()
        transactions_before = OrderProduct.objects.count()
        ...
        assert "Cannot assign order to finished transaction." in str(e.value)
        assert order_products_before == order_products_after
        assert transactions_before == transactions_after
        assert product.status == "available"
    ...
    def test_create_invalid_product_id(self):
        ...
        with pytest.raises(InvalidInputData) as e:
            self.service.create(self.token, self.user, self.order_product_creation_data)
        product = Product.objects.filter(product_id=self.product.product_id + 1).first()
        order_products_after = OrderProduct.objects.count()
        transactions_after = OrderProduct.objects.count()
    ...
    def test_create_product_seller_is_not_owner_of_product(self):
        self.seller2_data = {"email": "test4@example.com", "username": "tester4", "password": "Abc123#ab",
                            "is_verified": True, "bio": "I'm new here!", "money": 0.00, "is_superuser": False,
                            "last_login": None, "address": "fweffwe", "postal_code": "00001", "city": "Warsaw",
                            "country": "Poland"}
        AuthenticationHelper.register_and_login_user(self.seller2_data)
        seller2 = User.objects.get(username=self.seller2_data["username"])
        product = Product(owner=seller2, name="fefeffe", description="fefeffe", amount=1, price=1000,
                          color="red", weight=2, length=3, width=3, height=3, guarantee_period=1, status="available")
        product.save()
        self.order_product_creation_data["product_id"] = product.product_id
        order_products_before = OrderProduct.objects.count()
        transactions_before = OrderProduct.objects.count()
    ...
    def test_create_product_unavailable(self):
        product = Product.objects.get(product_id=self.order_product_creation_data["product_id"])
...
class TestDeleteOrderProductService:
    def setup_method(self):
        ...
        self.order_product = OrderProduct(product=product, transaction=transaction, seller=self.seller, shopping_price=item_price)
        self.order_product.save()
        self.order_product_id = self.order_product.order_product_id
        self.service = DeleteOrderProductService()
        self.delete_communicate = "Order deleted successfully"

    def test_delete_id(self):
        order_products_before = OrderProduct.objects.count()
        product_id = OrderProduct.objects.get(order_product_id=self.order_product_id).product.product_id
        result = self.service.delete(self.token, self.user, self.order_product_id)

        status = Product.objects.get(product_id=product_id).status
        order_products_after = OrderProduct.objects.count()

        assert self.delete_communicate == result
        assert order_products_before == order_products_after + 1
    ...
    def test_delete_user_is_not_owner_of_transaction(self):
        ...
        with pytest.raises(InvalidInputData) as e:
            self.service.delete(self.token_seller, self.seller, self.order_product_id)
        status = OrderProduct.objects.get(order_product_id=self.order_product_id).product.status
        order_products_after = OrderProduct.objects.count()
    ...
    def test_delete_id_expired_access_token(self):
        ...
        with pytest.raises(TokenExpiredError) as e:
            self.service.delete(access_token, self.user, self.order_product_id)
        status = OrderProduct.objects.get(order_product_id=self.order_product_id).product.status
        order_products_after = OrderProduct.objects.count()
    ...
    def test_delete_expired_by_replacement_access_token(self):
        ...
        with pytest.raises(TokenExpiredByReplacementError) as e:
            self.service.delete(access_token_first, self.user, self.order_product_id)
        order_products_after = OrderProduct.objects.count()
        status = OrderProduct.objects.get(order_product_id=self.order_product_id).product.status
