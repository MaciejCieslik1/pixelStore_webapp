...
class TestFindByIdProductPhoto:
    def setup_method(self):
        ...
        self.product_photo = ProductPhotoTestHelper.create_product_photo(product, "test_url")
...
@pytest.mark.django_db
class TestFindMainPhotoProductPhoto:
    def setup_method(self):
        self.client, self.user = create_api_client_with_user()
        product = ProductTestHelper.create_product(self.user)
        self.product_photo = ProductPhotoTestHelper.create_product_photo(product, "test_url")
        self.product_id = self.product_photo.product_id
        self.product_photo_data = {"product_photo_id": self.product_photo.product_photo_id,
            "product_id": self.product_photo.product.product_id, "image_url": self.product_photo.image_url,
            "is_main_photo": self.product_photo.is_main_photo}

    @patch("store.service.product_photo_service.FindMainPhotoProductPhotoService.find_main_photo")
    def test_find_main_photo_success(self, mock_find_by_id):
        mock_find_by_id.return_value = self.product_photo_data

        response = self.client.get(f"/product_photo/find_main_photo/{self.product_id}/")
    ...
    @patch("store.service.product_photo_service.FindMainPhotoProductPhotoService.find_main_photo")
    def test_find_main_photo_invalid_token(self, mock_find_by_id):
        mock_find_by_id.side_effect = IncorrectTokenError("Access token error.")
    ...
    @patch("store.service.product_photo_service.FindMainPhotoProductPhotoService.find_main_photo")
    def test_find_main_photo_expired_token(self, mock_find_by_id):
        mock_find_by_id.side_effect = TokenExpiredError("Access token error.")
    ...
    @patch("store.service.product_photo_service.FindMainPhotoProductPhotoService.find_main_photo")
    def test_find_main_photo_cannot_get_token_from_request(self, mock_find_by_id):
        mock_find_by_id.side_effect = CannotGetTokenFromRequestError("Access token error.")
    ...
    @patch("store.service.product_photo_service.FindMainPhotoProductPhotoService.find_main_photo")
    def test_find_main_photo_token_expired_by_replacement(self, mock_find_by_id):
        mock_find_by_id.side_effect = TokenExpiredByReplacementError("Access token error.")
    ...
    @patch("store.service.product_photo_service.FindMainPhotoProductPhotoService.find_main_photo")
    def test_find_main_photo_other_exception(self, mock_find_by_id):
        mock_find_by_id.side_effect = DatabaseError("DB connection failed")
    ...
    @patch("store.service.product_photo_service.FindMainPhotoProductPhotoService.find_main_photo")
    def test_find_main_photo_invalid_input_data(self, mock_find_by_id):
        mock_find_by_id.side_effect = InvalidInputData("Invalid input data provided.")
    ...
    def test_find_main_photo_invalid_serializer(self):
        self.product_id = 0
        response = self.client.get(f"/product_photo/find_main_photo/{self.product_id}/")
