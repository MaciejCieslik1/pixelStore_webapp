from unittest.mock import patch

import pytest
from django.db import DatabaseError
from rest_framework import status

from store.exceptions import IncorrectTokenError, TokenExpiredError, CannotGetTokenFromRequestError, \
    TokenExpiredByReplacementError, InvalidInputData
from store.helper_tests_classes.authentication_test_helper import create_api_client_with_user
from store.helper_tests_classes.product_review_test_helper import ProductReviewTestHelper
from store.helper_tests_classes.product_test_helper import ProductTestHelper


@pytest.mark.django_db
class TestFindByIdProductReview:
    def setup_method(self):
        self.client, self.user = create_api_client_with_user()
        product = ProductTestHelper.create_product(self.user)
        self.product_review = ProductReviewTestHelper.create_product_review(product, self.user)
        self.product_review_data = {"product_review_id": self.product_review.product_review_id,
            "product_id": self.product_review.product.product_id, "rating": self.product_review.rating,
            "description": self.product_review.description, "reviewer_username": self.product_review.reviewer.username,
            "review_date": self.product_review.review_date}

    @patch("store.service.product_review_service.FindByIdProductReviewService.find_by_id")
    def test_find_by_id_success(self, mock_find_by_id):
        mock_find_by_id.return_value = self.product_review_data

        response = self.client.get(f"/product_review/find_by_id/{self.product_review.product_review_id}/")

        assert response.status_code == status.HTTP_200_OK
        assert response.data == self.product_review_data
        mock_find_by_id.assert_called_once()

    @patch("store.service.product_review_service.FindByIdProductReviewService.find_by_id")
    def test_find_by_id_invalid_token(self, mock_find_by_id):
        mock_find_by_id.side_effect = IncorrectTokenError("Access token error.")

        response = self.client.get(f"/product_review/find_by_id/{self.product_review.product_review_id}/")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindByIdProductReviewService.find_by_id")
    def test_find_by_id_expired_token(self, mock_find_by_id):
        mock_find_by_id.side_effect = TokenExpiredError("Access token error.")

        response = self.client.get(f"/product_review/find_by_id/{self.product_review.product_review_id}/")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindByIdProductReviewService.find_by_id")
    def test_find_by_id_cannot_get_token_from_request(self, mock_find_by_id):
        mock_find_by_id.side_effect = CannotGetTokenFromRequestError("Access token error.")

        response = self.client.get(f"/product_review/find_by_id/{self.product_review.product_review_id}/")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindByIdProductReviewService.find_by_id")
    def test_find_by_id_token_expired_by_replacement(self, mock_find_by_id):
        mock_find_by_id.side_effect = TokenExpiredByReplacementError("Access token error.")

        response = self.client.get(f"/product_review/find_by_id/{self.product_review.product_review_id}/")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindByIdProductReviewService.find_by_id")
    def test_find_by_id_other_exception(self, mock_find_by_id):
        mock_find_by_id.side_effect = DatabaseError("DB connection failed")

        response = self.client.get(f"/product_review/find_by_id/{self.product_review.product_review_id}/")

        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert response.data["error"] == "Unexpected error."

    @patch("store.service.product_review_service.FindByIdProductReviewService.find_by_id")
    def test_find_by_id_invalid_input_data(self, mock_find_by_id):
        mock_find_by_id.side_effect = InvalidInputData("Invalid input data provided.")

        response = self.client.get(f"/product_review/find_by_id/{self.product_review.product_review_id}/")

        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.data["error"] == "Invalid input data provided."

    def test_find_by_id_invalid_serializer(self):
        product_review_id = 0
        response = self.client.get(f"/product_review/find_by_id/{product_review_id}/")

        assert response.status_code == status.HTTP_400_BAD_REQUEST


@pytest.mark.django_db
class TestFindAllProductReviews:
    def setup_method(self):
        self.client, self.user = create_api_client_with_user()
        self.product = ProductTestHelper.create_product(self.user)
        self.product_review1 = ProductReviewTestHelper.create_product_review(self.product, self.user)
        self.product_review1_data = {"product_review_id": self.product_review1.product_review_id,
                                    "product_id": self.product_review1.product.product_id,
                                    "rating": self.product_review1.rating,
                                    "description": self.product_review1.description,
                                    "reviewer_username": self.product_review1.reviewer.username,
                                    "review_date": self.product_review1.review_date}
        self.product_review2 = ProductReviewTestHelper.create_product_review(self.product, self.user)
        self.product_review2_data = {"product_review_id": self.product_review2.product_review_id,
                                     "product_id": self.product_review2.product.product_id,
                                     "rating": self.product_review2.rating,
                                     "description": self.product_review2.description,
                                     "reviewer_username": self.product_review2.reviewer.username,
                                     "review_date": self.product_review2.review_date}
        self.product_reviews_data = [self.product_review1_data, self.product_review2_data]


    @patch("store.service.product_review_service.FindAllProductReviewsService.find_all")
    def test_find_all_product_reviews_success(self, mock_find_all):
        mock_find_all.return_value = self.product_reviews_data

        response = self.client.get(f"/product_review/find_all_for_product/{self.product.product_id}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_200_OK
        assert response.data == self.product_reviews_data
        mock_find_all.assert_called_once()

    @patch("store.service.product_review_service.FindAllProductReviewsService.find_all")
    def test_find_all_for_product_invalid_token(self, mock_find_all):
        mock_find_all.side_effect = IncorrectTokenError("Access token error.")

        response = self.client.get(f"/product_review/find_all_for_product/{self.product.product_id}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindAllProductReviewsService.find_all")
    def test_find_all_for_product_expired_token(self, mock_find_all):
        mock_find_all.side_effect = TokenExpiredError("Access token error.")

        response = self.client.get(f"/product_review/find_all_for_product/{self.product.product_id}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindAllProductReviewsService.find_all")
    def test_find_all_for_product_cannot_get_token_from_request(self, mock_find_all):
        mock_find_all.side_effect = CannotGetTokenFromRequestError("Access token error.")

        response = self.client.get(f"/product_review/find_all_for_product/{self.product.product_id}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindAllProductReviewsService.find_all")
    def test_find_all_for_product_token_expired_by_replacement(self, mock_find_all):
        mock_find_all.side_effect = TokenExpiredByReplacementError("Access token error.")

        response = self.client.get(f"/product_review/find_all_for_product/{self.product.product_id}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindAllProductReviewsService.find_all")
    def test_find_all_for_product_other_exception(self, mock_find_all):
        mock_find_all.side_effect = DatabaseError("DB connection failed")

        response = self.client.get(f"/product_review/find_all_for_product/{self.product.product_id}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert response.data["error"] == "Unexpected error."

    @patch("store.service.product_review_service.FindAllProductReviewsService.find_all")
    def test_find_all_for_product_invalid_input_data(self, mock_find_all):
        mock_find_all.side_effect = InvalidInputData("Invalid input data provided.")

        response = self.client.get(f"/product_review/find_all_for_product/{self.product.product_id}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.data["error"] == "Invalid input data provided."


@pytest.mark.django_db
class TestFindAllFromUserProductReviews:
    def setup_method(self):
        self.client, self.user = create_api_client_with_user()
        self.product = ProductTestHelper.create_product(self.user)
        self.product_review1 = ProductReviewTestHelper.create_product_review(self.product, self.user)
        self.product_review1_data = {"product_review_id": self.product_review1.product_review_id,
                                    "product_id": self.product_review1.product.product_id,
                                    "rating": self.product_review1.rating,
                                    "description": self.product_review1.description,
                                    "reviewer_username": self.product_review1.reviewer.username,
                                    "review_date": self.product_review1.review_date}
        self.product_review2 = ProductReviewTestHelper.create_product_review(self.product, self.user)
        self.product_review2_data = {"product_review_id": self.product_review2.product_review_id,
                                     "product_id": self.product_review2.product.product_id,
                                     "rating": self.product_review2.rating,
                                     "description": self.product_review2.description,
                                     "reviewer_username": self.product_review2.reviewer.username,
                                     "review_date": self.product_review2.review_date}
        self.product_reviews_data = [self.product_review1_data, self.product_review2_data]


    @patch("store.service.product_review_service.FindAllFromUserProductReviewsService.find_all")
    def test_find_all_from_user_reviews_success(self, mock_find_all):
        mock_find_all.return_value = self.product_reviews_data

        response = self.client.get(f"/product_review/find_all_from_user/{self.user.username}/", {"page": 1, "page_size": 10})

        assert response.status_code == status.HTTP_200_OK
        assert response.data == self.product_reviews_data
        mock_find_all.assert_called_once()

    @patch("store.service.product_review_service.FindAllFromUserProductReviewsService.find_all")
    def test_find_all_from_user_reviews_invalid_token(self, mock_find_all):
        mock_find_all.side_effect = IncorrectTokenError("Access token error.")

        response = self.client.get(f"/product_review/find_all_from_user/{self.user.username}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindAllFromUserProductReviewsService.find_all")
    def test_find_all_from_user_reviews_expired_token(self, mock_find_all):
        mock_find_all.side_effect = TokenExpiredError("Access token error.")

        response = self.client.get(f"/product_review/find_all_from_user/{self.user.username}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindAllFromUserProductReviewsService.find_all")
    def test_find_all_from_user_reviews_cannot_get_token_from_request(self, mock_find_all):
        mock_find_all.side_effect = CannotGetTokenFromRequestError("Access token error.")

        response = self.client.get(f"/product_review/find_all_from_user/{self.user.username}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindAllFromUserProductReviewsService.find_all")
    def test_find_all_from_user_reviews_token_expired_by_replacement(self, mock_find_all):
        mock_find_all.side_effect = TokenExpiredByReplacementError("Access token error.")

        response = self.client.get(f"/product_review/find_all_from_user/{self.user.username}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.FindAllFromUserProductReviewsService.find_all")
    def test_find_all_from_user_reviews_other_exception(self, mock_find_all):
        mock_find_all.side_effect = DatabaseError("DB connection failed")

        response = self.client.get(f"/product_review/find_all_from_user/{self.user.username}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert response.data["error"] == "Unexpected error."

    @patch("store.service.product_review_service.FindAllFromUserProductReviewsService.find_all")
    def test_find_all_from_user_reviews_invalid_input_data(self, mock_find_all):
        mock_find_all.side_effect = InvalidInputData("Invalid input data provided.")

        response = self.client.get(f"/product_review/find_all_from_user/{self.user.username}/", {"page": 1, "page_size": 10} )

        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.data["error"] == "Invalid input data provided."


@pytest.mark.django_db
class TestCreateProductReview:
    def setup_method(self):
        self.client, self.user = create_api_client_with_user()
        self.product = ProductTestHelper.create_product(self.user)
        self.product_review_creation_data = {"product_id": self.product.product_id,
                                     "rating": 5,
                                     "description": "test description"}

        self.product_review_communicate = "Product review created successfully."

    @patch("store.service.product_review_service.CreateProductReviewService.create")
    def test_create_success(self, mock_create):
        mock_create.return_value = self.product_review_communicate

        response = self.client.post("/product_review/create/", data=self.product_review_creation_data, format="json")

        assert response.status_code == status.HTTP_200_OK
        assert response.data["msg"] == self.product_review_communicate
        mock_create.assert_called_once()

    @patch("store.service.product_review_service.CreateProductReviewService.create")
    def test_create_invalid_token(self, mock_create):
        mock_create.side_effect = IncorrectTokenError("Access token error.")

        response = self.client.post("/product_review/create/", data=self.product_review_creation_data, format="json")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.CreateProductReviewService.create")
    def test_create_expired_token(self, mock_create):
        mock_create.side_effect = TokenExpiredError("Access token error.")

        response = self.client.post("/product_review/create/", data=self.product_review_creation_data, format="json")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.CreateProductReviewService.create")
    def test_create_cannot_get_token_from_request(self, mock_create):
        mock_create.side_effect = CannotGetTokenFromRequestError("Access token error.")

        response = self.client.post("/product_review/create/", data=self.product_review_creation_data, format="json")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.CreateProductReviewService.create")
    def test_create_token_expired_by_replacement(self, mock_create):
        mock_create.side_effect = TokenExpiredByReplacementError("Access token error.")

        response = self.client.post("/product_review/create/", data=self.product_review_creation_data, format="json")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.CreateProductReviewService.create")
    def test_create_other_exception(self, mock_create):
        mock_create.side_effect = DatabaseError("DB connection failed")

        response = self.client.post("/product_review/create/", data=self.product_review_creation_data, format="json")

        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert response.data["error"] == "Unexpected error."

    @patch("store.service.product_review_service.CreateProductReviewService.create")
    def test_create_invalid_input_data(self, mock_create):
        mock_create.side_effect = InvalidInputData("Invalid input data provided.")

        response = self.client.post("/product_review/create/", data=self.product_review_creation_data, format="json")

        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert response.data["error"] == "Invalid input data provided."

    def test_create_invalid_serializer(self):
        self.product_review_creation_data["product_id"] = 0
        response = self.client.post("/product_review/create/", data=self.product_review_creation_data, format="json")

        assert response.status_code == status.HTTP_400_BAD_REQUEST


@pytest.mark.django_db
class TestDeleteProductReview:
    def setup_method(self):
        self.client, self.user = create_api_client_with_user()
        product = ProductTestHelper.create_product(self.user)
        self.product_review = ProductReviewTestHelper.create_product_review(product, self.user)
        self.product_review_id = self.product_review.product_review_id
        self.product_communicate = "Product review deleted successfully"

    @patch("store.service.product_review_service.DeleteProductReviewService.delete")
    def test_delete_success(self, mock_delete):
        mock_delete.return_value = self.product_communicate

        response = self.client.delete(f"/product_review/delete/{self.product_review_id}/")

        assert response.status_code == status.HTTP_200_OK
        assert response.data["msg"] == self.product_communicate
        mock_delete.assert_called_once()

    @patch("store.service.product_review_service.DeleteProductReviewService.delete")
    def test_delete_invalid_token(self, mock_delete):
        mock_delete.side_effect = IncorrectTokenError("Access token error.")

        response = self.client.delete(f"/product_review/delete/{self.product_review_id}/")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.DeleteProductReviewService.delete")
    def test_delete_expired_token(self, mock_delete):
        mock_delete.side_effect = TokenExpiredError("Access token error.")

        response = self.client.delete(f"/product_review/delete/{self.product_review_id}/")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.DeleteProductReviewService.delete")
    def test_delete_cannot_get_token_from_request(self, mock_delete):
        mock_delete.side_effect = CannotGetTokenFromRequestError("Access token error.")

        response = self.client.delete(f"/product_review/delete/{self.product_review_id}/")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.DeleteProductReviewService.delete")
    def test_delete_token_expired_by_replacement(self, mock_delete):
        mock_delete.side_effect = TokenExpiredByReplacementError("Access token error.")

        response = self.client.delete(f"/product_review/delete/{self.product_review_id}/")

        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data["error"] == "Access token error."

    @patch("store.service.product_review_service.DeleteProductReviewService.delete")
    def test_delete_other_exception(self, mock_delete):
        mock_delete.side_effect = DatabaseError("DB connection failed")

        response = self.client.delete(f"/product_review/delete/{self.product_review_id}/")

        assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
        assert response.data["error"] == "Unexpected error."

    @patch("store.service.product_review_service.DeleteProductReviewService.delete")
    def test_delete_invalid_input_data(self, mock_delete):
        mock_delete.side_effect = InvalidInputData("Invalid input data provided.")

        response = self.client.delete(f"/product_review/delete/{self.product_review_id}/")

        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.data["error"] == "Invalid input data provided."

    def test_delete_invalid_serializer(self):
        product_review_id = 0
        response = self.client.delete(f"/product_review/delete/{product_review_id}/")

        assert response.status_code == status.HTTP_400_BAD_REQUEST
